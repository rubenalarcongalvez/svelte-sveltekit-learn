<script>
  /* IMPORTANT: Podemos bindear un valor, de forma que no sea solo lectura para afuera, sino que se comparta. Es como model() en Angular */
	let { value = $bindable(''), onsubmit, element = $bindable() } = $props();

	const select = (num) => () => (value += num);
	const clear = () => (value = '');
</script>

<div class="keypad" bind:this={element}>
